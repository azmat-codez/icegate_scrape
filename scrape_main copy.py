...
# ---------------------------------------------------------
        ...
        for angle in angles:
            ...
            """
            I want to implement a robust check in captcha_text where check if captcha_text is not == len 6 then continue 
            if captcha_text is not isalnum continue and do some if 1 in captcha_text  and not get response then replase 1 with l if not work for l then do for I
            for all this check take time so you can hold session for max 5 min 
            """
